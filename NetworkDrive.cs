...
namespace aejw.Network
{
    ...
    public class NetworkDrive
    {
        ...
        private void zDisplayDialog(Window poParentForm, int piDialog)
        {
            int i = -1;
            //int iHandle = 0;
            IntPtr iHandle = (IntPtr)0;
            //get parent handle
            if (poParentForm != null)
            {
                iHandle = new WindowInteropHelper(poParentForm).Handle;
                //iHandle = poParentForm.Handle.ToInt32();
            ...
            if (piDialog == 1)
            {
                i = WNetConnectionDialog((int)iHandle, RESOURCETYPE_DISK);
            }
            else if (piDialog == 2)
            {
                i = WNetDisconnectDialog((int)iHandle, RESOURCETYPE_DISK);
            }
            if (i > 0) { throw new System.ComponentModel.Win32Exception(i); }
            //set focus on parent form
    ...
    public static class NetworkDriveWrapper
    {
        /// <summary>
    ...
    {
        public DiskInfo(string id, string pn)
        {
            DiskID = id;
            DiskProviderName = pn;
        ...
        public string DiskProviderName;
        public override string ToString()
        {
            ...
            if (String.IsNullOrEmpty(DiskID) || String.IsNullOrWhiteSpace(DiskID))
                ...
                _disk = DiskID;
            if (String.IsNullOrEmpty(DiskProviderName) || String.IsNullOrWhiteSpace(DiskProviderName))
                _provider = "";
            else
                _provider = "("+DiskProviderName+")";
            return String.Format("{0}{1}", _disk, _provider);
        }
    }
    public static class WmiDiskHelper
    {
        public static List<DiskInfo> GetDiskNames()
        {
            List<DiskInfo> _lstDI = new List<DiskInfo>();
            string strComputer = ".";
            ...
            {
                string _diskID =(string) objDisk["DeviceID"];
                string _diskProvider = objDisk["ProviderName"] == null ? "": (string)objDisk["ProviderName"];

                _lstDI.Add(new DiskInfo(_diskID, _diskProvider));
            
            }
            return _lstDI;
        }
        public static string ExtractDiskID(string DiskInfo)
        {
            if (String.IsNullOrEmpty(DiskInfo) || String.IsNullOrWhiteSpace(DiskInfo))
                return "";

            //Format: C:\(\\IP\ShareFolder)
            
            string[] separators = new string[] { "(", ")"};

            string[] substrings = DiskInfo.Split(separators, StringSplitOptions.None);
            
            

            return substrings.Count()>=1 ? substrings[0] : "";

        }
        public static string ExtractProviderName(string DiskInfo)
        {
            if (String.IsNullOrEmpty(DiskInfo) || String.IsNullOrWhiteSpace(DiskInfo))
                return "";
            ...
            return substrings.Count() >= 2 ? substrings[1] : "";
            

        }

        public static ObservableCollection<string> ToObservableCollection(this List<DiskInfo> diskinfolist)
        {
            var ret = new ObservableCollection<string>();
            if (diskinfolist.Count == 0) return null;
            foreach (var disk in diskinfolist)
            {
                ret.Add(disk.ToString());
            }
            return ret;
        }
    }
